defmodule Zorb.Interpreter.Types do
  @moduledoc false
  defmodule ZChar, do: (@behaviour Orb.CustomType; def wasm_type, do: :i32)
  defmodule ZWord, do: (@behaviour Orb.CustomType; def wasm_type, do: :i32)
  defmodule Address, do: (@behaviour Orb.CustomType; def wasm_type, do: :i32)
  defmodule PackedAddress, do: (@behaviour Orb.CustomType; def wasm_type, do: :i32)
  defmodule Object, do: (@behaviour Orb.CustomType; def wasm_type, do: :i32)
  defmodule Property, do: (@behaviour Orb.CustomType; def wasm_type, do: :i32)
  defmodule Variable, do: (@behaviour Orb.CustomType; def wasm_type, do: :i32)
end
...
defmodule Zorb.Interpreter do
  ...
  # Default Unicode Translation Table (Spec 3.8.5.2)
  @unicode_table [
    0x00E4, 0x00F6, 0x00FC, 0x00C4, 0x00D6, 0x00DC, 0x00DF, 0x00BB, 0x00AB, 0x00EB, 0x00EF, 0x00FF,
    0x00CB, 0x00CF, 0x00E1, 0x00E9, 0x00ED, 0x00F3, 0x00FA, 0x00FD, 0x00C1, 0x00C9, 0x00CD, 0x00D3,
    0x00DA, 0x00DD, 0x00E0, 0x00E8, 0x00EC, 0x00F2, 0x00F9, 0x00C0, 0x00C8, 0x00CC, 0x00D2, 0x00D9,
    0x00E2, 0x00EA, 0x00EE, 0x00F4, 0x00FB, 0x00C2, 0x00CA, 0x00CE, 0x00D4, 0x00DB, 0x00E5, 0x00C5,
    0x00F8, 0x00D8, 0x00E6, 0x00C6, 0x00E7, 0x00C7, 0x00F0, 0x00D0, 0x00F1, 0x00D1, 0x00F5, 0x00D5,
    0x00BF, 0x00A1, 0x00A3, 0x0153, 0x0152, 0x00AA, 0x00BA, 0x00E6, 0x00C6, 0x00F8, 0x00D8, 0x00E5,
    0x00C5, 0x00E7, 0x00C7, 0x00F0, 0x00D0, 0x00F1, 0x00D1, 0x00F5, 0x00D5, 0x00FE, 0x00DE, 0x00F0,
    0x00D0, 0x00F1, 0x00D1, 0x00F5, 0x00D5, 0x00FE, 0x00DE, 0x00A9, 0x2122, 0x20AC
  ]

global do
  @pc 0; @version 0; @sp 0; @fp 0; @csp 0; @stack_base 0x90000; @call_stack_base 0x98000
  @globals_base 0; @static_memory_base 0; @dictionary_base 0; @object_table_base 0; @object_table_start 0
  @abbreviations_base 0; @alphabet_shift 0; @abbrev_mode 0; @recursion_depth 0; @packed_address_shift 0
  @object_entry_size 0; @object_parent_offset 0; @object_sibling_offset 0; @object_child_offset 0; @object_property_table_offset 0
  @random_state 1; @story_len 0; @capabilities 0; @zscii_state 0; @zscii_high 0; @unicode_table_base 0; @current_font 1; @halted 0; @step_counter 0
end
...
Orb.Import.register(Zorb.Interpreter.ZIO)
  ...
  defw write_byte(addr: T.Address, val: I32) do
    ...
    if(I32.ge_u(addr, @static_memory_base), do: halt(4, @pc, 0), else: Memory.store!(I32.U8, addr, I32.band(val, 0xFF)))
  ...
  defwp get_arg_mask(
          t1: I32, t2: I32, t3: I32, t4: I32, t5: I32, t6: I32, t7: I32, t8: I32, omit: I32
        ),
        I32,
        mask: I32 do
    mask = I32.const(0)
    if I32.ne(t1, I32.const(3)), do: mask = I32.or(mask, I32.const(0x01))
    if I32.ne(t2, I32.const(3)), do: mask = I32.or(mask, I32.const(0x02))
    if I32.ne(t3, I32.const(3)), do: mask = I32.or(mask, I32.const(0x04))
    if I32.ne(t4, I32.const(3)), do: mask = I32.or(mask, I32.const(0x08))
    if I32.ne(t5, I32.const(3)), do: mask = I32.or(mask, I32.const(0x10))
    if I32.ne(t6, I32.const(3)), do: mask = I32.or(mask, I32.const(0x20))
    if I32.ne(t7, I32.const(3)), do: mask = I32.or(mask, I32.const(0x40))
    if I32.ne(t8, I32.const(3)), do: mask = I32.or(mask, I32.const(0x80))
    mask
  end
...
defw fetch_byte(), I32, val: I32 do
  val = read_byte(@pc); @pc = I32.add(@pc, 1); val
end
...
defw get_object_address(obj: T.Object), T.Address do
  ...
  defw get_object_sibling(obj: T.Object), T.Object do
    if(I32.le_u(@version, 3), do: read_byte(I32.add(get_object_address(obj), @object_sibling_offset)), else: read_word(I32.add(get_object_address(obj), @object_sibling_offset)))
  end
  defw get_object_child(obj: T.Object), T.Object do
    if(I32.le_u(@version, 3), do: read_byte(I32.add(get_object_address(obj), @object_child_offset)), else: read_word(I32.add(get_object_address(obj), @object_child_offset)))
  end
  defw set_object_parent(obj: T.Object, parent: T.Object) do
    if(I32.le_u(@version, 3), do: write_byte(I32.add(get_object_address(obj), @object_parent_offset), parent), else: write_word(I32.add(get_object_address(obj), @object_parent_offset), parent))
  end
  defw set_object_sibling(obj: T.Object, sib: T.Object) do
    if(I32.le_u(@version, 3), do: write_byte(I32.add(get_object_address(obj), @object_sibling_offset), sib), else: write_word(I32.add(get_object_address(obj), @object_sibling_offset), sib))
  end
  defw set_object_child(obj: T.Object, child: T.Object) do
    if(I32.le_u(@version, 3), do: write_byte(I32.add(get_object_address(obj), @object_child_offset), child), else: write_word(I32.add(get_object_address(obj), @object_child_offset), child))
  end
...
defwp get_prop_num(addr: T.Address), I32, b: I32 do
  b = read_byte(addr); if(I32.le_u(@version, 3), do: I32.band(b, 31), else: I32.band(b, 63))
end
...
defwp get_prop_data_size(addr: T.Address), I32, b: I32 do
  ...
  defw get_prop_value(obj: T.Object, prop: T.Property), I32, addr: T.Address do
    ...
    if(I32.eq(addr, 0), do: return(read_word(I32.add(@object_table_base, I32.shl(I32.sub(prop, 1), 1)))))
  ...
  defw put_prop_value(obj: T.Object, prop: T.Property, val: I32), addr: T.Address do
    addr = get_prop_address(obj, prop); if(I32.eq(addr, 0), do: return())
    if(I32.eq(get_prop_data_size(addr), 1), do: write_byte(addr, val), else: write_word(addr, val))
  end
  defw get_next_prop(obj: T.Object, prop: T.Property), I32, addr: T.Address do
    if(I32.eq(prop, 0), do: return(get_prop_num(skip_name(I32.add(get_prop_table_address(obj), 1)))))
    addr = get_prop_address(obj, prop); if(I32.eq(addr, 0), do: return(0))
  ...
  defw check_attribute(obj: T.Object, attr: I32), I32 do
    I32.band(read_byte(I32.add(get_object_address(obj), I32.shr_u(attr, 3))), I32.shl(1, I32.sub(7, I32.band(attr, 7))))
  end

  defw set_attribute(obj: T.Object, attr: I32, val: I32), addr: T.Address, b: I32 do
    addr = I32.add(get_object_address(obj), I32.shr_u(attr, 3)); b = read_byte(addr)
    b = if(val, do: I32.or(b, I32.shl(1, I32.sub(7, I32.band(attr, 7)))), else: I32.band(b, I32.xor(I32.shl(1, I32.sub(7, I32.band(attr, 7))), 0xFF)))
    write_byte(addr, b)
  end
  ...
  defw zscii_to_unicode(char: I32), I32, num: I32 do
    ...
    if I32.ne(@unicode_table_base, 0) do
      ...
      if(I32.lt_u(I32.sub(char, 155), num), do: return(read_word(I32.add(@unicode_table_base, I32.add(1, I32.shl(I32.sub(char, 155), 1))))))
      return(63)
    end
    if(I32.lt_u(I32.sub(char, 155), 94), do: return(read_word(I32.add(0x80000, I32.shl(I32.sub(char, 155), 1)))))
    63
  end
  defw unicode_to_zscii(uni: I32), I32, num: I32, i: I32 do
  ...
  defw decode_zchar(zchar: T.ZChar), old_pc: T.Address do
    ...
    if I32.gt_u(@abbrev_mode, 0) do
      old_pc = @pc; @pc = I32.shl(read_word(I32.add(@abbreviations_base, I32.shl(I32.add(I32.shl(I32.sub(@abbrev_mode, 1), 5), zchar), 1))), 1)
      @abbrev_mode = 0; print_zstring(0); @pc = old_pc; return()
    end
    if(I32.eq(zchar, 0), do: (ZIO.print_char(32); @alphabet_shift = 0; return()))
    ...
    if(I32.eq(zchar, 4), do: (@alphabet_shift = 1; return()))
    if(I32.eq(zchar, 5), do: (@alphabet_shift = 2; return()))
  ...
  defw print_zstring(addr: T.Address), word: I32, done: I32, s_sh: I32 do
    ...
    @alphabet_shift = s_sh; @abbrev_mode = I32.const(0); @recursion_depth = I32.sub(@recursion_depth, I32.const(1))
  end
...
defw do_random(range: I32), I32, val: I32 do
  if(I32.le_s(range, 0), do: (@random_state = 1; return(0)))
  @random_state = I32.add(I32.mul(@random_state, 1_103_515_245), 12_345)
  val = I32.band(I32.shr_u(@random_state, 16), 0x7FFF); I32.add(I32.rem_u(val, range), 1)
end
...
defw do_call(addr: T.PackedAddress, res: T.Variable, mask: I32, a1: I32, a2: I32, a3: I32, a4: I32, a5: I32, a6: I32, a7: I32, a8: I32), lc: I32, ac: I32, i: I32, ofp: I32, val: I32 do
  ...
  lc = read_byte(addr); ac = count_args_from_mask(mask); ofp = @fp; @fp = @csp
  push_call_stack(I32.band(@pc, 0xFFFF)); push_call_stack(I32.shr_u(@pc, 16)); push_call_stack(I32.or(I32.shl(mask, 8), res)); push_call_stack(ofp)
...
defw do_return(v: I32), ofp: I32, rpc: T.Address, var: T.Variable do
  @csp = @fp; rpc = I32.or(read_call_stack(@fp), I32.shl(read_call_stack(I32.add(@fp, 1)), 16))
  var = I32.band(read_call_stack(I32.add(@fp, 2)), 0xFF); ofp = read_call_stack(I32.add(@fp, 3)); @pc = rpc; @fp = ofp
    ...
    if I32.lt_u(i, max) do
      char = unicode_to_zscii(ZIO.read_char())
              if I32.ne(char, 13) do
                ...
                write_byte(I32.add(I32.add(buf, st), i), char); i = I32.add(i, 1); ILoop.continue()
              end
      
    end
  end
  if(I32.lt_u(@version, 5), do: write_byte(I32.add(I32.add(buf, st), i), 0), else: write_byte(I32.add(buf, 1), i))
  13
end
...
defw step(), nil, b: I32, opc: I32, t1: I32, t2: I32, o1: I32, o2: I32, o3: I32, o4: I32, o5: I32, o6: I32, o7: I32, o8: I32, m: I32 do
  @step_counter = I32.add(@step_counter, 1); if(I32.eq(I32.rem_u(@step_counter, 1009), 0), do: ZIO.log_step(0x300, @pc))
  b = fetch_byte()
      if I32.eq(I32.band(b, 0xC0), 0xC0) do
        opc = I32.band(b, 0x1F); t1 = fetch_byte()
        if I32.band(b, 0x20) do
          if I32.or(I32.eq(opc, 0x0C), I32.eq(opc, 0x1A)) do
            t2 = fetch_byte(); o1 = fetch_var_operand(I32.band(I32.shr_u(t1, 6), 3)); o2 = fetch_var_operand(I32.band(I32.shr_u(t1, 4), 3))
            o3 = fetch_var_operand(I32.band(I32.shr_u(t1, 2), 3)); o4 = fetch_var_operand(I32.band(t1, 3))
            o5 = fetch_var_operand(I32.band(I32.shr_u(t2, 6), 3)); o6 = fetch_var_operand(I32.band(I32.shr_u(t2, 4), 3))
            o7 = fetch_var_operand(I32.band(I32.shr_u(t2, 2), 3)); o8 = fetch_var_operand(I32.band(t2, 3))
            # execute_var8(opc, t1, t2, o1, o2, o3, o4, o5, o6, o7, o8); return()
            m = get_arg_mask(I32.band(I32.shr_u(t1, 6), 3), I32.band(I32.shr_u(t1, 4), 3), I32.band(I32.shr_u(t1, 2), 3), I32.band(t1, 3), I32.band(I32.shr_u(t2, 6), 3), I32.band(I32.shr_u(t2, 4), 3), I32.band(I32.shr_u(t2, 2), 3), I32.band(t2, 3), 0)
            do_call(unpack_address(o1), if(I32.eq(opc, 0x0C), do: fetch_byte(), else: 0xFF), m, o2, o3, o4, o5, o6, o7, o8, 0); return()
          end
        end
  
    m = get_arg_mask(I32.band(I32.shr_u(t1, 6), 3), I32.band(I32.shr_u(t1, 4), 3), I32.band(I32.shr_u(t1, 2), 3), I32.band(t1, 3), 3, 3, 3, 3, 0)
    if I32.eq(I32.band(b, 0x20), 0) do
      o1 = if(I32.eq(opc, 4) or I32.eq(opc, 5) or I32.eq(opc, 13), do: if(I32.eq(I32.band(I32.shr_u(t1, 6), 3), 2), do: read_variable_peek(fetch_byte()), else: fetch_var_ref_operand(I32.band(I32.shr_u(t1, 6), 3))), else: fetch_var_operand(I32.band(I32.shr_u(t1, 6), 3)))
      o2 = fetch_var_operand(I32.band(I32.shr_u(t1, 4), 3))
      _ = fetch_raw_operand(I32.band(I32.shr_u(t1, 2), 3)); _ = fetch_raw_operand(I32.band(t1, 3)); execute_2op(opc, o1, o2); return()
    end
    o1 = fetch_var_operand(I32.band(I32.shr_u(t1, 6), 3)); o2 = fetch_var_operand(I32.band(I32.shr_u(t1, 4), 3))
    o3 = fetch_var_operand(I32.band(I32.shr_u(t1, 2), 3)); o4 = fetch_var_operand(I32.band(t1, 3)); execute_var(opc, m, o1, o2, o3, o4); return()
  end
      if I32.eq(I32.band(b, 0xC0), 0x80) do
        if I32.eq(b, 0xBE) do
          if I32.ge_u(@version, I32.const(5)) do
            opc = fetch_byte(); t1 = fetch_byte()
            o1 = fetch_var_operand(I32.band(I32.shr_u(t1, 6), 3)); o2 = fetch_var_operand(I32.band(I32.shr_u(t1, 4), 3))
            o3 = fetch_var_operand(I32.band(I32.shr_u(t1, 2), 3)); o4 = fetch_var_operand(I32.band(t1, 3)); execute_ext(opc, o1, o2, o3, o4); return()
          end
        end
    ...
    if I32.ne(t1, 3) do
      opc = I32.band(b, 0x0F)
      o1 = if(I32.eq(opc, 5) or I32.eq(opc, 6) or I32.eq(opc, 14), do: if(I32.eq(t1, 2), do: read_variable_peek(fetch_byte()), else: fetch_var_ref_operand(t1)), else: fetch_operand(t1))
      execute_1op(opc, o1); return()
    end
  ...
  defw load_story(ptr: T.Address, len: I32), i: I32 do
    i = 0; @story_len = len; loop CLoop do (if(I32.lt_u(i, len), do: (Memory.store!(I32.U8, i, Memory.load!(I32.U8, I32.add(ptr, i))); i = I32.add(i, 1); CLoop.continue()))) end
  end
  ...
  defw init(st_off: T.Address), addr: T.Address do
    @version = read_byte(0); if(I32.eq(@pc, 0), do: @pc = read_word(6))
    @globals_base = read_word(0x0C); @dictionary_base = read_word(0x08); @object_table_base = read_word(0x0A); @static_memory_base = read_word(0x0E); @abbreviations_base = read_word(0x18)
    @sp = 0; @csp = 0; @fp = 0; @current_font = 1; @capabilities = ZIO.get_capabilities()
    if I32.ge_u(@version, I32.const(5)) do
      addr = read_word(I32.const(0x34))
      if I32.ne(addr, I32.const(0)) do
    ...
    if I32.le_u(@version, 3), do: (@packed_address_shift = 1; @object_entry_size = 9; @object_parent_offset = 4; @object_sibling_offset = 5; @object_child_offset = 6; @object_property_table_offset = 7; @object_table_start = I32.add(@object_table_base, 62)), else: (@packed_address_shift = 2; @object_entry_size = 14; @object_parent_offset = 6; @object_sibling_offset = 8; @object_child_offset = 10; @object_property_table_offset = 12; @object_table_start = I32.add(@object_table_base, 126))
    if(I32.eq(@version, 8), do: @packed_address_shift = 3)
    push_call_stack(0); push_call_stack(0); push_call_stack(0xFF); push_call_stack(0); @fp = 0
  end
  ...
  defw get_csp(), I32 do
    ...
    Memory.initial_data!(0x81000, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ \r0123456789.,!?_#'\"/\\-:()")
    Memory.initial_data!(0x80000, @unicode_table |> Enum.map(fn u -> <<u::16-big>> end) |> Enum.join())
  end
