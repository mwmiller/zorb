defmodule Zorb.ProverTest do
  ...
  test "dynamic Capsule compilation" do
    wasm_bytes = Zorb.Capsule.compile("test/fixtures/provers/czech.z5")
    assert is_binary(wasm_bytes)

    # Verify we can run it
    halt = run_wasm_capsule(wasm_bytes)
    assert halt == {0, 0, 0}
  end

  defp run_wasm_capsule(wasm_bytes) do
    owner = self()

    {:ok, agent} =
      Agent.start_link(fn -> %{instance: nil, buffer: [], halt: nil, owner: owner} end)

    imports = build_imports_for_test(agent)

    {:ok, instance} = Wasmex.start_link(%{bytes: wasm_bytes, imports: imports})

    # No need for memory/store here as we don't load_story manually
    {:ok, _} = Wasmex.call_function(instance, "init", [0xA0000])

    task = Task.async(fn -> loop_for_test(instance, agent, 0) end)
  ...
  defp build_imports_for_test(agent) do
    %{
      "zio" => %{
        "print_char" =>
          {:fn, [:i32], [],
        ...
        "read_char" => {:fn, [], [:i32], fn _ctx -> wait_for_input_for_test(agent) end},
        "get_random_seed" => {:fn, [], [:i32], fn _ctx -> :rand.uniform(0x7FFFFFFF) end},
        "get_capabilities" => {:fn, [], [:i32], fn _ctx -> 0x08 end},
        "halt" =>
          {:fn, [:i32, :i32, :i32], [],
  ...
  defp wait_for_input_for_test(agent) do
    case Agent.get(agent, fn s -> s.buffer end) do
      [char | rest] ->
        Agent.update(agent, fn s -> %{s | buffer: rest} end)
        if char == 10, do: 13, else: char

      [] ->
        receive do
          {:zorb_input_ready} -> wait_for_input_for_test(agent)
        after
          1000 -> wait_for_input_for_test(agent)
        end
    end
  end

  defp loop_for_test(instance, agent, steps) when steps < 10_000_000 do
    case Agent.get(agent, fn s -> s.halt end) do
      nil ->
        case Wasmex.call_function(instance, "run_steps", [1000], 60_000) do
          {:ok, _} -> loop_for_test(instance, agent, steps + 1000)
