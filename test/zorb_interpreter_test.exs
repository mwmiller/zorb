defmodule Zorb.InterpreterTest do
  ...
  test "basic add instruction" do
    ...
    # 0x0C: Globals = 0x0200
    header = <<
      3, 0, 0, 0, 0, 0, 1, 0, # 0x00-0x07 (PC at 0x06 is 0x0100)
      0, 0, 0, 0, 2, 0, 0, 0  # 0x08-0x0F (Globals at 0x0C is 0x0200)
    >>
  ...
  test "call and return" do
    ...
    code = <<0xE0, 0x3F, 0x01, 0x00, 16>> # call 0x0100 (packed 0x0200), store in G16

    # Routine at 0x0200: 
    # 1 local variable
    # rtrue (return 1)
    routine = <<1, 0, 0, 0xB0>> # 1 local, rtrue (0xB0 is 0OP opcode 0)
